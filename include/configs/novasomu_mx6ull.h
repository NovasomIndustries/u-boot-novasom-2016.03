...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"loadtest=fatload mmc ${mmcdev}:${mmcpart} ${initrd_addr} ${initrd_file}\0" \
	"runtest=dcache off;icache off;fatload mmc ${mmcdev}:${mmcpart} 0x00907000 ddr-test-uboot-jtag-mx6ull.bin;go 0x00907000\0" \
        "mmcloadbootenv=fatload mmc ${mmcdev}:${mmcpart}  ${loadaddr} ${bootenv}\0" \
	"yoctoargs=setenv bootargs console=${console},${baudrate} root=${mmcroot} ramdisk_size=96000 \0" \
	"yoctoboot=fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} ${image}; fatload mmc ${mmcdev}:${mmcpart} ${fdt_addr} ${fdt_file};run yoctoargs;bootz ${loadaddr} - ${fdt_addr};\0" \
